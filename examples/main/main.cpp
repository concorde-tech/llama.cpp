...
static void write_logfile(const llama_context *ctx, const gpt_params &params,
  ...
  yaml_dump_non_result_info(logfile, params, ctx, timestamp, input_tokens,
                            model_desc);
...
static std::string chat_add_and_format(struct llama_model *model,
  ...
  auto formatted = llama_chat_format_single(model, g_params->chat_template,
...
int main(int argc, char **argv) {
  ...
  if (params.conversation) {
    if (params.enable_chat_template) {
      LOG_INF("%s: chat template example:\n%s\n", __func__,
              llama_chat_format_example(model, params.chat_template).c_str());
    } else {
      LOG_INF(
  ...
  {
    auto prompt =
        (params.conversation && params.enable_chat_template &&
         !params.prompt.empty())
            ? chat_add_and_format(
                  model, chat_msgs, "system",
                  params
                      .prompt) // format the system prompt in conversation mode
            : params.prompt;
  ...
  if (!session_tokens.empty()) {
    ...
    if (params.prompt.empty() && n_matching_session_tokens == embd_inp.size()) {
      ...
      LOG_WRN("%s: session file has low similarity to prompt (%zu / %zu "
              "tokens); will mostly be reevaluated\n",
  ...
  LOG_DBG("recalculate the cached logits (check): embd_inp.size() %zu, "
          "n_matching_session_tokens %zu, embd_inp.size() %zu, "
          "session_tokens.size() %zu\n",
  ...
  /*            return (ctrl_type == CTRL_C_EVENT) ? (sigint_handler(SIGINT),
   * true) : false;*/
  /*        };*/
  /*        SetConsoleCtrlHandler(reinterpret_cast<PHANDLER_ROUTINE>(console_ctrl_handler),
   * true);*/
  /*#endif*/
  /*    }*/
  ...
  std::ostringstream assistant_ss; // for storing current assistant message,
  ...
  while ((n_remain != 0 && !is_antiprompt) || params.interactive) {
    ...
    if (!embd.empty()) {
      ...
      if (ga_n == 1) {
        ...
        if (n_past + (int)embd.size() >= n_ctx) {
          if (!params.ctx_shift) {
            LOG_DBG("\n\n%s: context full and context shift is disabled => "
                    "stopping\n",
                    __func__);
            break;
          } else {
            ...
            LOG_DBG("context full, swapping: n_past = %d, n_left = %d, n_ctx = "
    ...
    if ((int)embd_inp.size() <= n_consumed && !is_interacting) {
      ...
      while ((int)embd_inp.size() > n_consumed) {
        ...
        gpt_sampler_accept(smpl, embd_inp[n_consumed],
    ...
    if ((int)embd_inp.size() <= n_consumed) {
      ...
      if (llama_token_is_eog(model, gpt_sampler_last(smpl))) {
        ...
        if (params.interactive) {
          ...
          if (params.enable_chat_template) {
            chat_add_and_format(model, chat_msgs, "assistant",
                                assistant_ss.str());
          }
          is_interacting = true;
          LOG("\n");
      ...
      if (n_past > 0 && is_interacting) {
        ...
        if (buffer.length() > 1) {
          ...
          std::string user_inp =
              format_chat ? chat_add_and_format(model, chat_msgs, "user",
                                                std::move(buffer))
                          : std::move(buffer);
          // TODO: one inconvenient of current chat template implementation is
          // that we can't distinguish between user input and special tokens
          // (prefix/postfix)
          const auto line_pfx =
              ::llama_tokenize(ctx, params.input_prefix, false, true);
          const auto line_inp =
              ::llama_tokenize(ctx, user_inp, false, format_chat);
          const auto line_sfx =
              ::llama_tokenize(ctx, params.input_suffix, false, true);

          LOG_DBG("input tokens: %s\n", string_from(ctx, line_inp).c_str());

          // if user stop generation mid-way, we must add EOT to finish model's
          // last response
